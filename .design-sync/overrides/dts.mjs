// forked from design-sync lib/dts.mjs — raise the 120/140-char JSDoc truncation caps so prop docs don't lose their tail
...
function emitBody(type, at, generics, pkgDir) {
  ...
  for (const p of props) {
    ...
    if (doc) lines.push(`  /** ${doc.replace(/\s+/g, ' ').slice(0, 400)} */`);
    const pn = p.getName();
...
export function jsdocFor(name, ctx) {
  ...
  return doc.split('\n').find((l) => l.trim() && !l.trim().startsWith('@'))
    ?.trim().replace(/\s+/g, ' ').replace(/[^\w\s.,()'/:+-]/g, '').slice(0, 400) ?? '';
}
