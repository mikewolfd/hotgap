...
const rendered = async (page: Page) => {
  await expect(page.locator("#answer")).toContainText("Net income — pay plus help, after taxes, premiums and child care — stays below its $51,456 peak (at $36,000) until $45,000: 2 of the 9 $1,000 raises between them lose money");
...
test("es-US: the answer, the picture's labels and the disclosure names are the Spanish redraft", async ({ page }) => {
  ...
  for (const width of [390, 1280] as const) {
    ...
    await expect(page.locator("#answer")).toHaveText(/^El ingreso neto — sueldo más ayuda, después de impuestos, primas y cuidado infantil — se queda por debajo de su pico de \$[\d,]+ \(en \$[\d,]+\) hasta \$[\d,]+: \d+ de los \d+ aumentos de \$1,000 entre ambos pierden? dinero; con \$[\d,]+ más al año sale del tramo\.$/);
