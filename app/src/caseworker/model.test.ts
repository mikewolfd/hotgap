...
describe("the answer, one sentence in the caseworker register", () => {
  it("names the household's own zone, its exit and the raise that clears it — and nothing else", () => {
    ...
    expect(answerText(co)).toBe("Net stays below its $51,456 peak (at $36,000) until $45,000: 2 of the 9 steps between them lose money; $7,000 clears the stretch.");
  ...
  it("has one sentence for every curve shape", () => {
    ...
    expect(shape({}, { raiseIsLowerBound: true })).toBe("Net stays below its $51,456 peak (at $36,000) past the top of the axis, $150,000: 8 of the 114 steps between them lose money.");
