...
describe("verdict shapes", () => {
  ...
  test("cliff ahead: the threshold is the step's landing point, the drop about-rounded", () => {
    ...
    expect(verdictText(s)).toMatch(/^You are paid \$38,000 a year\. You keep \$50,400\. Near \$42,000, more pay can mean less money: past it you would keep about \$2,500 less a year\. People call this a benefits cliff\. One more thing: at \$72,000/);
...
describe("the person's unit (M5)", () => {
  test("hourly: every pay figure to $0.25 an hour, the leap the difference of the rounded figures, money kept yearly", () => {
    ...
    expect(verdictText(s)).toMatch(/^You are paid \$23\.75 an hour\. You keep \$51,100\. More pay does not add to that until you are paid \$25\.25 an hour: a raise of \$1\.50 an hour\. One more thing: at \$39\.50 an hour,/);
  });
  test("monthly: to $50 a month, the unit said on every figure", () => {
    const s = sceneOf(makeEvaluation(), { unit: "month" });
    expect(verdictText(s)).toMatch(/^You are paid \$3,600 a month\. You keep \$51,100\. More pay does not add to that until you are paid \$3,850 a month: a raise of \$250 a month\. One more thing: at \$6,000 a month,/);
  });
...
describe("the deferred clause (B1)", () => {
  test("a deferred loss at the person's own pay is in the answer, not two screens down", () => {
    // The Head Start case: the deferred step starts at the person's pay.
    const s = sceneOf(makeEvaluation({}, 71_000), year);
    expect(s.deferred[0].startEarnings).toBe(71_000);
    expect(verdictText(s)).toMatch(/One more thing: at \$72,000, your kids' free state health plan stops, but not that day\. Later, you would keep about \$1,500 less a year\.$/);
  });
  test("no deferred cliff at or above the pay, no clause", () => {
    expect(verdictText(sceneOf(makeEvaluation({}, 80_000), year))).not.toMatch(/One more thing/);
    expect(verdictText(sceneOf(makeEvaluation({}, 43_000, { deferredCliff: false }), year))).not.toMatch(/One more thing/);
  });
});
