...
| State $0-premium marketplace tiers (CT, MA, NM, CA) modeled locally | `core/src/statePremiumWraps.ts`, `core/src/evaluate.ts` `applyPremiumWrap` | the endpoint serves the state's own amount. Since 2026-09-16 the client probes each endpoint for the state's tax-unit variable (`core/src/statePremiumAssistance.ts`: CA, NJ, NM, MD, CO, VT, WA) and, where served, nets PolicyEngine's amount out of the premium and stands the ladder down for that state — the engine (2.6.2) serves all seven, the hosted API (1.764.6) none. Compared on 2.5.0: CA within $4 of the ladder; NM's upstream model keeps paying 250–400% FPL where the ladder stopped. CT and MA: the doc used to say they had no upstream model (issue #9481) — no longer true. The engine at 2.6.2 serves `ct_covered_connecticut` and `ma_connector_care` too (probed 2026-09-16; on a single parent of two CT pays the whole residual, $2,162 at $44,000, and MA up to $4,008), so both could join `STATE_PREMIUM_ASSISTANCE` and stand their ladders down where served; not done here because it changes their curves and needs a CT/MA resweep of its own. Until then their ladders apply everywhere. |
