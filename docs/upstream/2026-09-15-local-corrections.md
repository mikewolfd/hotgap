...
| Employee ESI contribution replaces the marketplace premium | `core/src/evaluate.ts` `applyEmployerCoverage` | upstream models the employee share (issue #9473). 2.5.0 already has the inputs to do this upstream-side: `has_esi: true` on the person zeroes the PTC and the marketplace premium, and `other_health_insurance_premiums` carries the employee share into `spm_unit_medical_out_of_pocket_expenses` (CA adult at $30k: PTC $5,895 / premium $1,850 → $0 / $1,789). Sending those two in place of the local replacement is the retire path once the public API has them too. |
...
| State $0-premium marketplace tiers (CT, MA, NM, CA) modeled locally | `core/src/statePremiumWraps.ts`, `core/src/evaluate.ts` `applyPremiumWrap` | the state subsidy reaches the premium HotGap reads. Checked on policyengine-us 2.5.0 (2026-09-15): upstream now computes CA (`assigned_ca_premium_subsidy`), NM, MD and CO amounts, but only into `household_health_benefits` → `household_net_income_including_health_benefits`; `marketplace_net_premium` is still plan cost − federal PTC, so `spm_unit_medical_out_of_pocket_expenses` carries the full federal-net premium on both endpoints and the wrap is not redundant (issue #9481). A later step could read those upstream amounts in place of the local ladders where the endpoint serves them. |
