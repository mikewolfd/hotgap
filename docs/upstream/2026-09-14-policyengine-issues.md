...
| https://github.com/PolicyEngine/policyengine-us/pull/9478 — count MA TAFDC once in household benefits | #9470 | open: `ma_tafdc` was the only `STATE_TANF_VARIABLES` member also listed in `household_state_benefits`; removed from all four dated lists, with a test that fails on the old parameter. 1,160 MA + partner tests pass. |
