...
| PR | Fixes | Status |
|---|---|---|
| https://github.com/PolicyEngine/policyengine-us/pull/9475 — 2026 parent Medicaid limits in six frozen-standard states | #9474 | maintainer-reviewed 2026-09-16: every 2026 figure reproduced; SC is pegged to the guideline (67%, $1,842.50 for four = 0.67 × $33,000 ÷ 12), so the stale 2021 `1.00` was deleted rather than overridden; TX partner case re-pinned. We re-verified all four adjustments against the state sources and added the pending 2025 waiver resubmission to the SC comment. Awaiting merge. |
| https://github.com/PolicyEngine/policyengine-us/pull/9499 — 2026 Idaho parent Medicaid limit 0.27 → 0.21 (IDAPA 16.03.01.411, $365/month for three) | #9474 | **merged 2026-09-16**, with #9475 the same day. Idaho is an expansion state so this moves the category, not coverage. |
| https://github.com/PolicyEngine/policyengine-us/pull/9504 — Maryland CCS provider type defaults to a licensed center, not NONE | #9485 | open. HotGap names the type itself meanwhile (translate.ts). |
| https://github.com/PolicyEngine/policyengine-us/pull/9505 — Massachusetts CCFA provider type follows the child's age category when not named | #9485 | open. Same. |
| https://github.com/PolicyEngine/policyengine-us/pull/9506 — no marketplace premium for a person in the Medicaid coverage gap | #9472 | open, on the gate the maintainer specified (under the PTC floor, no Medicaid pathway, no below-poverty exception). When merged and served, `applyCoverageGap` retires. |
| NY Basic Health Program end date (comment: https://github.com/PolicyEngine/policyengine-us/issues/9471#issuecomment-5681112987) | #9471 | no PR: `is_basic_health_program_eligible` is an annual variable that reads the January 1 parameter value, so a `2026-07-01: []` entry is inert for 2026 (the file's own comment already says so). The fix needs sub-annual handling in the variable, a maintainer decision; HotGap's local override models the post-July rule for the whole annual scenario. |
| https://github.com/PolicyEngine/policyengine-us/pull/9477 — MA TAFDC work-expense deduction (eligibility change withdrawn 2026-09-16) | #9469 | draft, now the per-earner deduction only: the maintainer showed 704.280, not 704.281, is the applicant test's disregard and the code implements it. With the deduction counted once DTA's own $7,512 example fails the applicant test, so the recipient-vs-applicant design call is open on the issue; HotGap models the ongoing recipient locally either way. Was: 542 MA tests pass; one partner contract expectation (`partners/analytics_coverage/.../snap/ma.yaml`) legitimately changes because SNAP now sees the corrected TAFDC, which needs a maintainer's sign-off. The eligibility test applies 704.281(B)'s 50% disregard but not 704.281(A)'s six-month full disregard (pointing it at the grant's blended income made a $45,600 family eligible). Exit is $30,960; DTA's $7,512 example reproduces exactly. |
| https://github.com/PolicyEngine/policyengine-us/pull/9478 — count MA TAFDC once in household benefits | #9470 | **merged 2026-09-15**; retire the double-count workaround when the API serves it. Was: `ma_tafdc` was the only `STATE_TANF_VARIABLES` member also listed in `household_state_benefits`; removed from all four dated lists, with a test that fails on the old parameter. 1,160 MA + partner tests pass. |
