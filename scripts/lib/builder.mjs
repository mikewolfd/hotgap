...
// download to disk and `unzip -p` streamed out of an archive. Nothing here
// knows what a builder builds.
...
/** `--out=path` resolved against the working directory, else the builder's default under core/data. */
export const outputPath = (argv, defaultUrl) => (argv.out ? new URL(argv.out, `file://${process.cwd()}/`) : defaultUrl);

/**
 * Writes `{ [stampKey]: stamp, ...body }`, keeping the file's existing stamp
 * when the body has not changed — the convention summary.json set: the stamp
 * is the run that last CHANGED the numbers, so an unchanged rebuild is not a
 * diff. Returns the body that was there before (empty when there was none).
 */
...
/**
 * One entry of a zip as a stream, in bounded memory: `stdout` to read,
 * `closed` to await once it is drained (rejects if unzip did).
 */
