...
describe("committed data", () => {
  ...
  it("every state file exists with a full-axis curve for every archetype", () => {
    for (const state of STATE_CODES) {
      const file = loadStateFile(state);
      expect(file?.state, state).toBe(state);
      for (const a of ARCHETYPES) expect(file!.archetypes[a.id].points, `${state} ${a.id}`).toHaveLength(axisSpec(answersFor(state, a)).count);
    }
  });
